...
class SaveOmnivaIntTerminalToNewOrderObserver implements ObserverInterface
{
    /**
    ...
    public function execute(EventObserver $observer)
    {
        ...
        if (isset($params['order']['omniva_int_terminal'])){
            $quote = $this->getQuote();
            $quote_address = $quote->getShippingAddress();
            if ($quote_address){
                $quote_address->setOmnivaIntTerminal( $params['order']['omniva_int_terminal']);
                $quote_address->save();
            }
        }
        return $this;
