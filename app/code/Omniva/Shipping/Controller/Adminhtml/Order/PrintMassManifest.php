...
class PrintMassManifest extends \Magento\Sales\Controller\Adminhtml\Order\AbstractMassAction
{
     ...
     */
    public function __construct(Context $context, Filter $filter, CollectionFactory $collectionFactory, OrderManagementInterface $orderManagement, \Omniva\Shipping\Model\Carrier $omniva_carrier) {
    ...
    private function _fillDataBase(AbstractCollection $collection) {
        ...
        foreach ($collection->getItems() as $order) {
            ...
            if (!$this->isOmnivaMethod($order)) {
                $text = 'Warning: Order ' . $order->getData('increment_id') . ' not Omniva shipping method.';
    ...
    public function massAction(AbstractCollection $collection) {
        ...
        $name = $this->omniva_carrier->getConfigData('cod_company');
        $phone = $this->omniva_carrier->getConfigData('company_phone');
        $street = $this->omniva_carrier->getConfigData('company_address');
        $postcode = $this->omniva_carrier->getConfigData('company_postcode');
        $city = $this->omniva_carrier->getConfigData('company_city');
        $country = $this->omniva_carrier->getConfigData('company_countrycode');
        ...
        try {
            ...
            foreach ($pack_data as $order) {
                ...
                if (strtoupper($order->getData('shipping_method')) == strtoupper('omniva_PARCEL_TERMINAL')) {
                    $shippingAddress = $order->getShippingAddress();
                    $terminal_id = $shippingAddress->getOmnivaIntTerminal();
                    $order_address = $this->omniva_carrier->getTerminalAddress($terminal_id);
                } else {
            ...
            if ($count > 0) {
                $manifest->downloadManifest('D', 'Omniva_manifest_' . date('Y-m-d H.i.s'));
            } else {
                $this->_redirect($this->_redirect->getRefererUrl());
                return;
            }
