...
class PrintMassLabels extends \Magento\Sales\Controller\Adminhtml\Order\AbstractMassAction
{
    ...
    public function massAction(AbstractCollection $collection) {
        ...
        try {
            ...
            if (!empty($this->labelsContent)) {
                ...
                $outputPdf->Output('Omniva_labels.pdf', 'D');
                return;
                //return $fileFactory->create('OmnivaShippingLabels.pdf', $outputPdf->Output('S'), DirectoryList::VAR_DIR, 'application/pdf');
            }
        } catch (\Exception $e) {
    ...
    public function _createShipment($order) {
        ...
        if ($order->getId() && !empty($shipmentItems)) {
            ...
            $order->addStatusHistoryComment('Automatically SHIPPED by Omniva mass action.', false);
            $order->save();
        } else {
    ...
    private function _combineLabelsPdf(array $labelsContent) {
        ...
        $count = 1;
        $labels = 4;
        $combine = $this->omnivaCarrier->getConfigData('combine_labels');
